...
class do_trade(QThread):
    ...
    save_auto_top = pyqtSignal(pd.DataFrame)
    val_light = pyqtSignal(bool)
    val_wallet = pyqtSignal(int,int)
    val_ror_sum = pyqtSignal(str)
    val_time = pyqtSignal(str)
    shutdown_signal = pyqtSignal()
    q_message = pyqtSignal(str,str)
    def __init__(self, parent, dict_bybit, dict_binance, df_open, df_closed, df_set,
                 df_linear, dict_option, df_top_ticker):
        super().__init__(parent)
        self.cond = QWaitCondition()
        self.bool_light = False
        self._status = True
        self.linear_benefit = '0'
        self.dict_bybit = dict_bybit
        self.dict_binance = dict_binance
        self.df_top_ticker = df_top_ticker
    ...
    def run(self):
        ...
        start_time = self.dict_option['start_time'].replace(minute=0, second=0, microsecond=0)
        next_hour = start_time + datetime.timedelta(hours=1)
        self.fetch_balance()
        df_top_bybit = pd.DataFrame()
        df_top_binance = pd.DataFrame()


        if self.dict_bybit['active'] == True:
            df_bybit = self.df_inverse.loc[self.df_inverse['market'] == 'bybit']
            sumary = df_bybit['합계(USD)'].sum()
            if sumary > 500:
                df_top_bybit = self.get_top('bybit',self.df_top_ticker) #상위만 매수
            else:
                print(f'bybit 잔고 부족, 현재잔고: {sumary}')
        if self.dict_binance['active'] == True:
            df_bybit = self.df_inverse.loc[self.df_inverse['market'] == 'binance']
            sumary = df_bybit['합계(USD)'].sum()
            if sumary > 500:
                df_top_binance = self.get_top('binance',self.df_top_ticker) #상위만 매수
            else:
                print(f'binance 잔고 부족, 현재잔고: {sumary}')

        self.df_top_ticker = pd.concat([df_top_bybit, df_top_binance], axis=0).astype(self.df_top_ticker.dtypes)
        print(f"run \n {self.df_top_ticker}")
        self.save_auto_top.emit(self.df_top_ticker)
        # self.only_sell() #전부다 매도
        ...
        for idx,row in self.df_future.iterrows():
            if idx in self.df_linear.index.tolist():
                if row['진입수량'] < self.df_linear.loc[idx,'진입수량']:
                    self.df_linear.loc[idx, '진입수량'] = row['진입수량']
                    # self.df_linear.loc[idx, '매수금액'] = self.df_linear.loc[idx, '진입수량']+self.df_linear.loc[idx, '평단가']
                    self.df_linear.loc[idx, '매수금액'] = self.df_linear.loc[idx, '진입수량']+row['진입가']
        ...
        while self._status:
            ...
            self.check_get_top(now_time=now_time,bet=55)
            if self.dict_bybit['active'] == True:
            ...
            self.text_time = funding_time - now_time
            ...
            self.change_price()
            if now_time > next_hour:# 매 정시마다
                if now_time > funding_time:
                    self.df_set.loc['funding_time', 'val'] = self.common.get_funding_time(now_time)
                    self.save_set.emit(self.df_set)
                # print('잔고조회')
                next_hour = datetime.datetime.now().replace(minute=0, second=0, microsecond=0) + datetime.timedelta(hours=1)
                self.check_time = True
            if self.check_time == True:# 매 정시마다 잔고 조회
                self.fetch_balance()
            ...
            for idx in self.df_inverse.index.tolist():
                ...
                if ticker == 'USDT':
                    ...
                    continue
                if self.check_time == True:# 매 정시마다 linear 매수 조회
                    pass
                    # dict_txt_inverse = self.buy_auto(idx,market,ticker,dict_txt_inverse)
                    # print(f'buy auto and linear 조회 : {market} - {ticker}')
                    ...
                    #
                    # open_long_signal, 진입횟수, 신호번호 = self.get_open_long_signal(df, market, ticker)
                    # now_time = datetime.datetime.now().replace(microsecond=0)
                    # if open_long_signal == True:
                    #     print(f"run - 진입_long 신호 감지 | {now_time} {market= }   {ticker= }  |  {진입횟수= }  {신호번호=}")
                    #     dict_txt_linear = self.open_linear(idx=idx,division=dict_division.get(ticker,50),
                    #                                future_leverage=dict_leverage.get(ticker,2),
                    #                                       dict_txt=dict_txt_linear,direction = 'long')
                    #
                    # open_short_signal = self.get_open_short_signal(df, market, ticker)
                    # if open_short_signal == True:
                    #     print(f"run - 진입_short 신호 감지 | {now_time} {market= }   {ticker= }  |  {진입횟수= }  {신호번호=}")
                    #     dict_txt_linear = self.open_linear(idx=idx,division=dict_division.get(ticker,50),
                    #                                future_leverage=dict_leverage.get(ticker,2),
                    #                                       dict_txt=dict_txt_linear,direction = 'short')
                    #
                    # if f"{market}_{ticker}_long" in self.df_linear.index.tolist():
                    #     # lx = [x[:x.rindex("_")] for x in self.df_linear.index.tolist()]
                    #     # if idx in lx:
                    #     close_long_signal, 매도수량, 매수횟수,신호번호 = self.get_close_long_signal(df, market, ticker)
                    #     if close_long_signal == True:
                    #         print(f"run - 청산_long 신호 감지 | {now_time} {market= }   {ticker= }  |  {매도수량= }   {매수횟수= }  {신호번호=}")
                    #         self.close_linear(df=df, market=market,ticker=ticker, direction='long',매도수량=매도수량,매수횟수=매수횟수)
                    # if f"{market}_{ticker}_short" in self.df_linear.index.tolist():
                    #     close_short_signal, 매도수량, 매수횟수 = self.get_close_short_signal(df, market, ticker)
                    #     if close_short_signal == True:
                    #         print(f"run - 청산_short 신호 감지 | {now_time} {market= }   {ticker= }  |  {매도수량= }   {매수횟수= }")
                    #         self.close_linear(df=df, market=market,ticker=ticker, direction='short',매도수량=매도수량,매수횟수=매수횟수)
            if dict_txt_linear:
                print('- linear 진입 -')
            ...
            if datetime.datetime.now() > finish_time:
                # 윈도우 종료
                if len(self.df_top_ticker.index.tolist()) > 1: #top_ticker행 이외에 다른게 있을 경우
                    # print(f'df_top_ticker 의 티커가 있습니다. 시스템 종료를 취소합니다.')
                    pass
                else:
                    base_date = datetime.datetime.now().date()
                    time1 = datetime.datetime.combine(base_date, datetime.datetime.now().time()).time()
                    stock_start_time = datetime.datetime.combine(base_date, time1.replace(hour=8, minute=30)).time()
                    stock_end_time = datetime.datetime.combine(base_date, time1.replace(hour=15, minute=0)).time()
                    if stock_end_time < time1 or time1 <  stock_start_time:
    ...
    def change_price(self):
        ...
        if not self.df_future.empty:
            for index, row in self.df_future.iterrows():
                ...
                if row['방향'] == 'long':
                    수익률 = round((현재가-row['진입가'])/row['진입가']*row['레버리지']*100,2)
                    수익금 = round((현재가-row['진입가'])*row['진입수량'],1)
                elif row['방향'] == 'short':
                    수익률 = round((row['진입가']-현재가)/row['진입가']*row['레버리지']*100,2)
                    수익금 = round((row['진입가']-현재가)*row['진입수량'],1)
        ...
        if not self.df_linear.empty:
            for index, row in self.df_linear.iterrows():
                ...
                if row['market'] == 'bybit':
                    ...
                    현재가 = self.tickers_binance[f"{ticker}/USDT"]['close']
                #################################
                # 평단가 = row['매수금액']*row['레버리지']/row['진입수량']
                if row['방향'] == 'long':
                    수익률 = (현재가-row['평단가'])/row['평단가']*row['레버리지']*100
                    수익금 = (현재가-row['평단가'])*row['진입수량']
                    # 수익률 = (현재가-평단가)/평단가*row['레버리지']*100
                    # 수익금 = (현재가-평단가)*row['진입수량']
                elif row['방향'] == 'short':
                    수익률 = (row['평단가']-현재가)/row['평단가']*row['레버리지']*100
                    수익금 = (row['평단가']-현재가)*row['진입수량']
                    # 수익률 = (평단가-현재가)/평단가*row['레버리지']*100
                    # 수익금 = (평단가-현재가)*row['진입수량']
                self.df_linear.loc[index,'현재가'] = 현재가
                self.df_linear.loc[index,'수익률'] = round(수익률,2)
                self.df_linear.loc[index,'수익금'] = round(수익금,2)
            self.linear_benefit = int(self.df_linear['수익금'].sum())
    ...
    def buy_auto(self, idx, market, ticker,dict_txt):
        ...
        # if self.df_inverse.loc[idx,'배팅가능합계(USD)'] > 주문최소금액*2:
        #     진입수량 = self.df_inverse.loc[idx,'배팅가능합계(USD)']
#         print(f"{ticker=}  {price= } = {현재가= } + {(현재가 * (rate_short / 100))= }")
#         print(f"{여유돈= } = {보유코인합계= }*{funding_rate * 안전마진= }")
#         print(f"{배팅가능금액= } = {배팅가능합계= } - {여유돈= }")
#         print(f"{진입수량= } = math.trunc(배팅가능금액)")
        ...
        if order : # 현재 잔고가 진입수량*펀딩비율*5배 보다 많아야 매수 조건 성립 (최소수량보다 잔고가 많을경우마다 주문하면 마이너스피 일 때는 갖고있는 잔고에서 매번 수수료가 나가기 때문)
            if market == 'binance':
                ...
                signal = True
                # print(self.df_top_ticker)
                df_ticker = self.df_top_ticker.loc[self.df_top_ticker.index==ticker]
                # print(df_ticker)
                if not df_ticker.empty:
                    if df_ticker['상태'][0] == '과매수' or df_ticker['상태'][0] == '과매도':
                        signal = False
                # signal = False
                # df = self.common.get_df(market, ticker, '60분봉', 300)  # 일봉의 이평이 데드크로스일 때만 신규 진입
                # if df.loc[df.index[-1], '이평200'] < df.loc[df.index[-1], '종가']:
                #     signal = True
            if self.except_tickers: #신규 매수는 무조건 매수 시그날
                if f"{market}_{ticker}" in self.except_tickers:
                    signal = True
            if signal == True:
                ...
                if self.except_tickers: # 신규 매수는 시장가로
                    print(f"{self.except_tickers= }  {market}_{ticker}")
                    if f"{market}_{ticker}" in self.except_tickers:
                        per = 1
                        while True:
                            amount = 진입수량*per
                            per = per - 0.001
                            print(f"{amount=}   {진입수량=}   {per=} ")
                            try:
                                res = self.common.order_open(market=market, category='inverse', ticker=ticker, side='sell',
                                                     orderType="market", price=주문가, qty=amount)
                            except Exception as e:
                                print(e)
                        while True:
                            ord_closed = self.common.fetch_closed_orders('bybit',res['id'],row["ticker"],'inverse')
                            print(f"ord_closed:  {ord_closed= }")
                            if ord_closed['info'].get('status')=='FILLED' or ord_closed['info'].get('orderStatus')=='Filled':
                                break
                        self.except_tickers.remove(f"{market}_{ticker}")
                else:
                    res = self.common.order_open(market=market, category='inverse', ticker=ticker, side='sell',
    ...
    def buy_manual(self,market,ticker,bet_amount,df_inverse):
        print("현물 매수 요청 수신!")
        df_open = pd.DataFrame()
        category = 'spot'
        ...
        fee = 0.1
        레버리지 = 1
        진입수량 = (100 - (fee * 레버리지)) / 100 * bet_amount / 주문가
        진입수량 = self.common.amount_precision(market=market, category=category,ticker=ticker, amount=진입수량)
        ...
        if 보유현금 < 매수금액:
            print(f"USDT 부족 - {market=} {ticker=} 보유한 USDT: {보유현금}, 필요한 USDT: {매수금액}  |  {market= }  {ticker= }   {bet_amount= }  {rate_spot= }")
            self.q_message.emit('에러',f"USDT 부족 -{market} {ticker} 보유한 USDT: {보유현금}, 필요한 USDT: {매수금액}")
            return 0
        else:
            ...
            if market == 'binance':
                ...
                if 매수금액 < min_usd:
                    print(f'{market= } {ticker= } {매수금액= } 매수금액이 더 필요합니다.')
                    return 0
        res = self.common.order_open(market= market,category=category, ticker=ticker, side='buy', orderType="market",
                              price=주문가, qty=진입수량)
        id = res['id']
        df_open.loc[id, 'market'] = market
        df_open.loc[id, 'ticker'] = ticker
    ...
    def open_linear(self, idx, division, future_leverage, dict_txt,direction):
        market = self.df_inverse.loc[idx, 'market']
        ticker = self.df_inverse.loc[idx, 'ticker']
        ...
        print(f"open_linear 진입신호 : {now_time}  {market= }   {ticker= }  {idx}  {used_usdt * future_leverage}   {min_amount_future=}")
        #
        if used_usdt * future_leverage < min_amount_future:
            ...
            return  # 진입수량이 선물 최소주문수량보다 작거나 인버스 주문최소금액보다 작을 경우 pass
        ...
        while True:
            ...
            i = i + 1
            if i > 10:
                print(f'{market} open_linear 에러 1  ')
                quit()
        ...
        if market == 'binance':
            ...
            before_qty = res[ticker]['free']
            self.common.transfer_to(market=market, ticker=ticker, amount=free_qty, departure='inverse',
                                    destination='spot')
            for i in range(20):
                QTest.qWait(1000)
                res = self.common.dict_binance['spot'].fetch_balance()
                if not before_qty == res[ticker]['free']:
                    free_qty = res[ticker]['free']*0.9
                    print(f"{market} 자금이동 완료  {now_time}  {ticker= }   {before_qty=}    {free_qty=}")
                    QTest.qWait(1000)
                    break
                print(f"open_linear - [{i}] {market= } {ticker= } 자금 이동 중: {free_qty=}  <=  {res[ticker]['free']= } ")
        elif market == 'bybit':
            res = self.common.dict_bybit['exchange'].fetch_balance()
            free_qty = res[ticker]['free'] * 0.9
            print(f"open_linear _ {market= }: {free_qty}  <=  {res[ticker]['free']= } {ticker= }")
            QTest.qWait(1000)

        category = 'spot'
        free_qty = self.common.amount_precision(market, category, ticker, free_qty)
        ...
        id = res['id']
        while True:
            dict_chegyeol = self.fetch_order(market=market, ticker=ticker, id=id, category=category,
                                             qty=free_qty)
            if dict_chegyeol['체결'] == True:
            ...
            QTest.qWait(1000)
            if i > 10:
                print(f'{market}  open_linear 에러 2')
                quit()

        usdt = dict_chegyeol['체결금액']
        if market == 'binance':
            self.common.transfer_to(market=market, ticker='USDT', amount=usdt, departure='spot',
                                    destination='linear')

        qty = (usdt * future_leverage) / price
        category = 'linear'
        qty = self.common.amount_precision(market, category, ticker, qty)
        res = self.common.order_open(market=market, category=category, ticker=ticker, side=direction,
                                     orderType='market', price=price, qty=qty, leverage=future_leverage)
        id = res['id']
        idx = idx + '_' + direction  # long일경우 'bybit_BTC_long'
        while True:
            ...
            if dict_chegyeol['체결'] == True:
                ...
                if not idx in self.df_linear.index.tolist():  # 신규 진입
                    ...
                    self.df_linear.loc[idx, '진입수량'] = dict_chegyeol['체결수량']
                    self.df_linear.loc[idx, '매수금액'] = round(
                        (dict_chegyeol['체결가'] * dict_chegyeol['체결수량']) / future_leverage)
                    ...
                    self.df_linear.loc[idx, '방향'] = direction
                else: # 추가 진입
                    기존진입수량 = float(self.df_linear.loc[idx, '진입수량'])
                    ...
                    self.df_linear.loc[idx, '주문수량'] = qty
                    self.df_linear.loc[idx, '체결가'] = dict_chegyeol['체결가']
                    self.df_linear.loc[idx, '진입수량'] = 기존진입수량 + dict_chegyeol['체결수량']
                    self.df_linear.loc[idx, '매수금액'] = round(기존매수금액 + (dict_chegyeol['체결가'] * dict_chegyeol['체결수량']) / future_leverage)
                    평단가 = (self.df_linear.loc[idx, '매수금액']*future_leverage) / self.df_linear.loc[idx, '진입수량']
                ...
                break
            if i > 10:
                print(f'{market= }   {ticker= }   open_linear 에러 3')
                quit()
            QTest.qWait(1000)
        dict_txt[id] = {}
        dict_txt[id]['market'] = market
    ...
    def get_top(self,market,df_top_ticker):
        start_time = self.dict_option['start_time'] #프로그램 시작시간
        # df_top_ticker['주문시간'] = pd.to_datetime(df_top_ticker['주문시간'])
        # df_top_ticker['만료시간'] = pd.to_datetime(df_top_ticker['만료시간'])
        # top_ticker_time = df_top_ticker.loc['top_ticker','주문시간']
        top_ticker_expire = pd.to_datetime(df_top_ticker.loc[market,'만료시간'])
        df_top_ticker = df_top_ticker.loc[df_top_ticker['market'] == market]
        if top_ticker_expire < start_time: # 10일 뒤에 list_top 갖고오기
            top = 10 # 상위 10개
            list_top = self.common.get_inverse_TOP(market=market,days=30,ratio=1.5,top=top) #result가 0.00018 이상인 티커만
            # list_top = self.common.get_inverse_TOP(market='binance',days=90,top=1.5) #result가 0.00018 이상인 티커만
            self.fetch_balance()
            df_inverse = self.df_inverse.loc[self.df_inverse['market'] == market]
            df_inverse_ticker = df_inverse.loc[df_inverse['ticker'] != 'USDT']
            df_inverse_ticker.index = df_inverse_ticker[f'ticker']
            list_inverse = df_inverse_ticker['ticker'].tolist()
            # df_top_ticker = pd.DataFrame(index=list_top+['top_ticker'], columns=['market','합계(USD)','목표(USD)','상태','주문_category','주문수량','id','주문시간'])
            # df_top_ticker['주문시간'] = start_time

            new_ticker = list(set(list_top) - set(list_inverse))
            del_ticker = list(set(list_inverse) - set(list_top))
            hold_ticker = list(set(list_inverse) & set(list_top))
            sumary = df_inverse['합계(USD)'].sum()
            if sumary < 500:
                bet_division = 0
            else:
                bet_USDT = sumary*0.9 # 배팅가능 USDT의 90%만 배팅
                # bet_division = bet_USDT//len(list_top)
                bet_division = bet_USDT//top #배팅규모 나누기 5
            if bet_division > 1000: # 1000 이상일 경우 1000으로 고정
                bet_division = 1000
            for ticker in hold_ticker: # 기존에서 보유물량
                df_top_ticker.loc[f"{market}_{ticker}",'ticker'] = ticker
                df_top_ticker.loc[f"{market}_{ticker}",'합계(USD)'] = df_inverse_ticker.loc[f"{ticker}",'합계(USD)']
                df_top_ticker.loc[f"{market}_{ticker}",'목표(USD)'] = bet_division
                df_top_ticker.loc[f"{market}_{ticker}",'상태'] = '보유'
            for ticker in new_ticker: # 신규
                df_top_ticker.loc[f"{market}_{ticker}",'ticker'] = ticker
                df_top_ticker.loc[f"{market}_{ticker}",'합계(USD)'] = 0
                df_top_ticker.loc[f"{market}_{ticker}",'목표(USD)'] = bet_division
                df_top_ticker.loc[f"{market}_{ticker}", '상태'] = '신규'
            for ticker in del_ticker: # 제거
                df_top_ticker.loc[f"{market}_{ticker}",'ticker'] = ticker
                df_top_ticker.loc[f"{market}_{ticker}",'합계(USD)'] = df_inverse_ticker.loc[f"{ticker}",'합계(USD)']
                df_top_ticker.loc[f"{market}_{ticker}",'목표(USD)'] = 0
                df_top_ticker.loc[f"{market}_{ticker}", '상태'] = '제거'
            df_top_ticker['market'] = market
            df_top_ticker = self.convert_column_types(df_top_ticker)
            for idx, row in df_top_ticker.iterrows():
                if idx == "bybit" or idx == "binance":
                    # df_top_ticker.loc[idx, '주문시간'] = start_time
                    df_top_ticker.loc[idx, '만료시간'] = start_time+datetime.timedelta(days=10)
                    df_top_ticker.loc[idx, 'ticker'] = "USDT"
                    df_top_ticker.loc[idx, 'market'] = market
                else:
                    df_top_ticker.loc[idx,'market'] = market
                    # df_top_ticker.loc[idx,'주문시간'] = start_time
                    if row['합계(USD)'] > row['목표(USD)']:
                        df_top_ticker.loc[idx,'상태'] = '과매수'
                        df_top_ticker.loc[idx,'주문_category'] = 'inverse'
                        # df_top_ticker.loc[idx,'id'] = np.nan
                    elif row['합계(USD)'] < row['목표(USD)']:
                        df_top_ticker.loc[idx,'상태'] = '과매도'
                        df_top_ticker.loc[idx,'주문_category'] = 'spot'
#                         df_top_ticker.loc[idx,'id'] = np.nan
                    else:
                        print(f"{idx}  {row['합계(USD)']= }   ")
                        df_top_ticker.drop(index=idx,inplace=True)
                    df_top_ticker.loc[idx, 'ticker'] = row['ticker']
            # self.save_auto_top.emit(df_top_ticker)

        return df_top_ticker
    def only_sell(self):
        start_time = self.dict_option['start_time']
        self.df_top_ticker['주문시간'] = pd.to_datetime(self.df_top_ticker['주문시간'])
        self.df_top_ticker['만료시간'] = pd.to_datetime(self.df_top_ticker['만료시간'])
        top_ticker_expire = self.df_top_ticker.loc['top_ticker','만료시간']
        if top_ticker_expire < start_time: # 10일 뒤에 list_top 갖고오기
            print('onlyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy')
            # list_top = self.common.get_inverse_TOP(market='bybit',days=30,top=1.9) #result가 0.00018 이상인 티커만
            list_top = []
            self.fetch_balance()
            df_inverse_bybit = self.df_inverse.loc[self.df_inverse['market'] == 'bybit']
            df_inverse_ticker = df_inverse_bybit.loc[df_inverse_bybit['ticker'] != 'USDT']
            df_inverse_ticker.index = df_inverse_ticker['ticker']
            list_inverse = df_inverse_ticker['ticker'].tolist()

            del_ticker = list(set(list_inverse) - set(list_top))

            self.df_top_ticker = pd.DataFrame(index=['top_ticker'],
                                              columns=['market', '합계(USD)', '목표(USD)', '상태', '주문_category', '주문수량',
                                                       '주문가', 'id', '주문시간', '만료시간'])
            for ticker in del_ticker: # 제거
                self.df_top_ticker.loc[ticker,'합계(USD)'] = df_inverse_ticker.loc[ticker,'합계(USD)']
                self.df_top_ticker.loc[ticker,'목표(USD)'] = 0
                self.df_top_ticker.loc[ticker, '상태'] = '제거'

            for idx, row in self.df_top_ticker.iterrows():
                if idx == "top_ticker":
                    self.df_top_ticker.loc[idx, '주문시간'] = start_time
                    self.df_top_ticker.loc[idx, '만료시간'] = start_time+datetime.timedelta(days=10)
                else:
                    self.df_top_ticker.loc[idx,'market'] = 'bybit'
                    self.df_top_ticker.loc[idx,'주문시간'] = start_time
                    self.df_top_ticker.loc[idx,'상태'] = '과매수'
                    self.df_top_ticker.loc[idx,'주문_category'] = 'inverse'
                    self.df_top_ticker.loc[idx,'id'] = ''

            self.save_auto_top.emit(self.df_top_ticker)
    def check_get_top(self,now_time,bet):
        if len(self.df_top_ticker.index.tolist()) > 2: #top_ticker행 이외에 다른게 있을 경우
            self.df_top_ticker['주문시간'] = pd.to_datetime(self.df_top_ticker['주문시간'])
            self.df_top_ticker['만료시간'] = pd.to_datetime(self.df_top_ticker['만료시간'])
            for idx, row in self.df_top_ticker.iterrows():
                ticker = row['ticker']
                # if '과매도' in self.df_top_ticker['상태'].tolist():
                #     print("과매도가 생기면 안되는데 과매도가 생김")
                #     quit()
                market = row['market']
                if ticker == "USDT":
                    continue
                    ...
                    if row['id']=='': #주문
                        print('===========================')
                        if row['주문_category'] == 'inverse':
                            positions = self.common.fetch_position(market,row['주문_category'],ticker)
                            amount = abs(float(positions['contracts']))  # inverse 포지션 수량
                            # amount = self.df_inverse.loc[f'bybit_{ticker}','합계(USD)']
                            if amount > bet:
                                amount = bet
                            rate = -0.05
                            현재가 = self.common.fetch_ticker(market=market, ticker=f"{ticker}USD")['close']
                            price = 현재가 * (1 + (rate / 100))
                            price = self.common.price_to_precision(market=market, category=row['주문_category'], ticker=ticker,price=price)
                            if amount < 5:
                                self.df_top_ticker.drop(index=idx, inplace=True)
                                self.save_auto_top.emit(self.df_top_ticker)
                                continue

                            res = self.common.order_close(market=market, category=row['주문_category'], ticker=ticker, side='buy',
                                                   orderType="limit", price=price, qty=amount)
                            res['주문수량'] = amount
                            res['주문가'] = price
                            res['매수금액'] = amount
                        elif row['주문_category'] == 'spot':
                            # positions = self.dict_bybit['exchange'].fetch_balance({'type': 'spot'})
                            # amount = self.df_inverse.loc[f'bybit_{ticker}','합계(USD)']
                            positions = self.common.fetch_position(market,row['주문_category'],ticker)
                            amount = float(positions['free']) * 0.9  # 90프로만 주문 (안그러면 잔고 에러 뜸)
                            amount = self.common.amount_precision(market=market, category=row['주문_category'], ticker=ticker,
                                                           amount=amount)
                            # price = self.dict_bybit['exchange'].fetch_ticker(f"{ticker}/USDT")['close']
                            현재가 = self.common.fetch_ticker(market=market, ticker=f"{ticker}/USDT")['close']
                            rate = 0.05
                            price = 현재가 * (1 + (rate / 100))
                            price = self.common.price_to_precision(market=market, category=row['주문_category'], ticker=ticker, price=price)
                            res = self.common.order_close(market=market, category=row['주문_category'], ticker=ticker, side="sell",
                                                   orderType="limit", price=price, qty=amount)
                            res['주문수량'] = amount
                            res['주문가'] = price
                            res['매수금액'] = price * amount
                        print(f"check_get_top 과매수 {ticker}   {row['주문_category']=} {amount=} ,{price=} {res['id']=}")
                        self.df_top_ticker.loc[idx,'id'] = res['id']
                        self.df_top_ticker.loc[idx, '주문시간'] = now_time
                        self.df_top_ticker.loc[idx, '만료시간'] = now_time+datetime.timedelta(minutes=240) #주문후 4시간 경과 시 취소
                        self.df_top_ticker.loc[idx,'주문수량'] = res['주문수량']
                        self.df_top_ticker.loc[idx,'주문가'] = res['주문가']
                        self.save_auto_top.emit(self.df_top_ticker)
                        df_open = pd.DataFrame()
                        id = res['id']
                        df_open.loc[id, 'market'] = market
                        df_open.loc[id, 'ticker'] = ticker
                        df_open.loc[id, '주문시간'] = now_time.strftime("%Y-%m-%d %H:%M")
                        df_open.loc[id, '주문수량'] = res['주문수량']
                        df_open.loc[id, 'id'] = id
                        df_open.loc[id, '매수금액'] = res['매수금액']
                        df_open.loc[id, '상태'] = row['상태']
                        df_open.loc[id, 'category'] = row['주문_category']
                        df_open.loc[id, '주문가'] = res['주문가']
                        if not self.df_open.empty:
                            self.df_open = pd.concat([self.df_open, df_open], axis=0).astype(self.df_open.dtypes)
                        else:
                            self.df_open = df_open.copy()
                        ...
                        if row['주문_category'] == 'inverse':
                            dict_chegyeol = self.fetch_order(market=market, ticker=ticker, id=row['id'], category=row['주문_category'],
                                                             qty=row['주문수량'])
                            if dict_chegyeol['체결'] == True:
                                print(f"===체결 {market=} spot  {ticker=}   {row['id']=}  {dict_chegyeol}")
                                self.df_open.drop(index=row['id'], inplace=True)
                                self.qt_open.emit(self.df_open)
                                self.df_top_ticker.loc[idx,'id'] = ''
                                self.df_top_ticker.loc[idx,'주문_category'] = 'spot'
                                self.fetch_balance()
                                self.df_top_ticker.loc[idx,'합계(USD)'] = self.df_inverse.loc[f"{market}_{ticker}",'합계(USD)']
                                self.save_auto_top.emit(self.df_top_ticker)
                            elif dict_chegyeol['체결'] == '주문취소':
                                print(f"취소됨 {dict_chegyeol= } {idx= }   {row['상태']= }  {row['주문_category']=}   {row['id']=} ")
                                print(self.df_top_ticker)

                                if row['id'] in self.df_open.index.tolist():
                                    self.df_open.drop(index=row['id'], inplace=True)
                                    self.qt_open.emit(self.df_open)
                                self.df_top_ticker.loc[idx,'id'] = ''
                                self.save_auto_top.emit(self.df_top_ticker)

                            elif row['만료시간'] < now_time:
                                if row['id'] in self.df_open.index.tolist():
                                    self.df_open.drop(index=row['id'], inplace=True)
                                    self.qt_open.emit(self.df_open)
                                self.common.order_cancel(market=market,category=row['주문_category'],ticker=ticker,order_id=row['id'])
                                print(f"주문취소 {dict_chegyeol= } {market=} {row['주문_category']=}  {idx=}   {row['id']=}")

                                self.df_top_ticker.loc[idx,'id'] = ''
                                self.save_auto_top.emit(self.df_top_ticker)
                            elif dict_chegyeol['체결'] == '보류':
                                print(f"보류 1 {dict_chegyeol= } {ticker= }   {row['상태']= }  {row['주문_category']=}   {row['id']=} ")
                            ...
                            dict_chegyeol = self.fetch_order(market=market, ticker=ticker, id=row['id'], category=row['주문_category'],
                                                             qty=row['주문수량'])
                            if dict_chegyeol['체결'] == True:
                                print(f"===체결 {market=} {row['주문_category']=}  {ticker=}   {row['id']=}  {dict_chegyeol}")

                                self.df_open.drop(index=row['id'], inplace=True)
                                self.qt_open.emit(self.df_open)
                                self.df_top_ticker.loc[idx,'id'] = ''
                                self.fetch_balance()
                                sum_usd = self.df_inverse.loc[f"{market}_{ticker}",'합계(USD)']
                                self.df_top_ticker.loc[idx,'합계(USD)'] = sum_usd
                                tolerance = 0.10 # 10%
                                # lower_bound = self.df_top_ticker.loc[idx, '목표(USD)'] * (1 - tolerance)
                                upper_bound = float(self.df_top_ticker.loc[idx, '목표(USD)']) * (1 + tolerance)
                                if sum_usd <= upper_bound:
                                    print(f"과매수 조건 만족 {idx= } {sum_usd=} {upper_bound}")
                                    # print(self.df_top_ticker)
                                    self.df_top_ticker.drop(index=idx,inplace=True)
                                else:
                                    self.df_top_ticker.loc[idx,'주문_category'] = 'inverse'
                                    print(f"과매수 조건 불만족 {idx= }")
                                    # print(self.df_top_ticker)
                                self.save_auto_top.emit(self.df_top_ticker)
                                ...
                                if row['id'] in self.df_open.index.tolist():
                                    ...
                                    self.qt_open.emit(self.df_open)
                                self.common.order_cancel(market=market,category=row['주문_category'],ticker=ticker,order_id=row['id'])
                                print(f"주문취소 {dict_chegyeol= }  {market=} {row['주문_category']=}  {idx=}  {row['상태']= }  {row['id']=}  ")

                                self.df_top_ticker.loc[idx,'id'] = ''
                                self.save_auto_top.emit(self.df_top_ticker)
                            elif dict_chegyeol['체결'] == '보류':
                                print(f"보류 2 {dict_chegyeol= } {ticker= }   {row['상태']= }  {row['주문_category']=}   {row['id']=} ")
                elif row['상태']=='과매도':
                    if row['id']=='' or row['id']==None or pd.isna(row['id']): # 주문
                        if row['목표(USD)'] == 0:
                            continue
                        print('===========================')
                        if row['주문_category'] == 'spot':
                            배팅금액 = bet
                            side = "buy"
                            현재가 = self.common.fetch_ticker(market=market, ticker=ticker + '/USDT')['close']
                            rate = -0.2
                            주문가 = 현재가 * (1+(rate/100))
                            주문가 = self.common.price_to_precision(market=market, category=row['주문_category'], ticker=ticker,
                                                                 price=주문가)
                            주문수량 = 배팅금액 / 주문가
                            주문수량 = self.common.amount_precision(market=market, category=row['주문_category'], ticker=ticker,
                                                                amount=주문수량)
                            매수금액 = 주문수량*주문가
                        elif row['주문_category'] == 'inverse':
                            side = "sell"
                            self.fetch_balance()
                            주문수량 = int(self.df_inverse.loc[f"{market}_{ticker}",'배팅가능합계(USD)'])
                            # if 주문수량 <50: # 가끔 최소주문수량이 안되는데 어딘가 꼬여서 추가 함
                            #     self.df_top_ticker.loc[idx, 'id'] = ''
                            #     self.df_top_ticker.loc[idx, '주문_category'] = 'spot'
                            #     return
                            rate = 0.2
                            현재가 = self.common.fetch_ticker(market=market, ticker=ticker + 'USD')['close']
                            주문가 = 현재가 * (1+(rate/100))

                            주문가 = self.common.price_to_precision(market=market, category=row['주문_category'], ticker=ticker,
                                                           price=주문가)
                            매수금액 = 주문수량
                        print(f"check_get_top 과매도 {idx}   {row['주문_category']=} {주문수량=} {현재가=}   {주문가=} {매수금액=}")
                        res = self.common.order_open(market=market, category=row['주문_category'], ticker=ticker, side=side,
                                                     orderType="limit",
                                                     price=주문가, qty=주문수량)
                        print(res['id'])
                        self.df_top_ticker.loc[idx,'id'] = res['id']
                        self.df_top_ticker.loc[idx, '주문시간'] = now_time
                        self.df_top_ticker.loc[idx, '만료시간'] = now_time+datetime.timedelta(minutes=240) #주문후 4시간 경과 시 취소
                        self.df_top_ticker.loc[idx,'주문수량'] = 주문수량
                        self.df_top_ticker.loc[idx,'주문가'] = 주문가
                        self.save_auto_top.emit(self.df_top_ticker)
                        print(self.df_top_ticker)
                        df_open = pd.DataFrame()
                        id = res['id']
                        ...
                        df_open.loc[id, 'ticker'] = ticker
                        df_open.loc[id, '주문시간'] = now_time.strftime("%Y-%m-%d %H:%M")
                        df_open.loc[id, '주문수량'] = 주문수량
                        df_open.loc[id, 'id'] = id
                        df_open.loc[id, '매수금액'] = 매수금액
                        df_open.loc[id, '상태'] = row['상태']
                        df_open.loc[id, 'category'] = row['주문_category']
                        df_open.loc[id, '주문가'] = 주문가
                        if not self.df_open.empty:
                        ...
                        if row['주문_category'] == 'spot':
                            dict_chegyeol = self.fetch_order(market=market, ticker=ticker, id=row['id'], category=row['주문_category'],
                                                             qty=row['주문수량'])
                            if dict_chegyeol['체결'] == True:  #inverse 주문은 buy_auto에서 알아서 주문나감
                                print(f"===체결 {market=} spot  {ticker=}   {row['id']=}  {dict_chegyeol}")
                                self.df_open.drop(index=row['id'], inplace=True)
                                self.qt_open.emit(self.df_open)
                                self.df_top_ticker.loc[idx,'id'] = ''
                                ...
                                if row['id'] in self.df_open.index.tolist():
                                    self.df_open.drop(index=row['id'], inplace=True)
                                    self.qt_open.emit(self.df_open)
                                self.common.order_cancel(market=market,category='spot',ticker=ticker,order_id=row['id'])
                                print(f"주문취소 {dict_chegyeol= }  {market=} {row['주문_category']=}  {ticker=}  {row['상태']= }  {row['id']=}  ")


                                self.df_top_ticker.loc[idx,'id'] = ''
                                self.save_auto_top.emit(self.df_top_ticker)
                            elif dict_chegyeol['체결'] == '보류':
                                print(f"보류 3 {dict_chegyeol= } {idx= }   {row['상태']= }  {row['주문_category']=}   {row['id']=} ")
                        elif row['주문_category'] == 'inverse':
                            dict_chegyeol = self.fetch_order(market=market, ticker=ticker, id=row['id'], category=row['주문_category'],
                                                             qty=row['주문수량'])
                            if dict_chegyeol['체결'] == True:  #inverse 주문은 buy_auto에서 알아서 주문나감
                                ...
                                self.df_top_ticker.loc[idx,'id'] = ''
                                self.fetch_balance()
                                sum_usd = self.df_inverse.loc[f"{market}_{idx}", '합계(USD)']
                                self.df_top_ticker.loc[idx, '합계(USD)'] = sum_usd
                                tolerance = 0.10
                                lower_bound = float(self.df_top_ticker.loc[idx, '목표(USD)']) * (1 - tolerance)
                                # upper_bound = self.df_top_ticker.loc[idx, '목표(USD)'] * (1 + tolerance)
                                if lower_bound <= sum_usd:
                                    print(f"과매수 조건 만족 {idx= }")
                                    # print(self.df_top_ticker)
                                    self.df_top_ticker.drop(index=idx, inplace=True)
                                else:
                                    self.df_top_ticker.loc[idx,'주문_category'] = 'spot'
                                    print(f"과매수 조건 불만족 {idx= } {sum_usd=} {lower_bound=}")
#                                     print(self.df_top_ticker)
                                self.save_auto_top.emit(self.df_top_ticker)
                                ...
                                if row['id'] in self.df_open.index.tolist():
                                    self.df_open.drop(index=row['id'], inplace=True)
                                    self.qt_open.emit(self.df_open)
                                self.common.order_cancel(market=market,category='inverse',ticker=ticker,order_id=row['id'])
                                print(f"주문취소 {dict_chegyeol= } {market=} {row['주문_category']=}  {idx=}   {row['id']=}")
                                self.df_top_ticker.loc[idx,'id'] = ''
                                self.save_auto_top.emit(self.df_top_ticker)
                            elif dict_chegyeol['체결'] == '보류':
                                print(f"보류 4 {dict_chegyeol= } {idx= }   {row['상태']= }  {row['주문_category']=}   {row['id']=} ")

    def close_linear(self,df,market,ticker,direction,매도수량,매수횟수):
        idx = f"{market}_{ticker}_{direction}"
        ...
        수익률 = self.df_linear.loc[idx,'수익률']
        현재가 = df.loc[df.index[-1],'종가']

        category = 'linear'
        매도수량 = self.common.amount_precision(market, category, ticker, 매도수량)
        if 매도수량 > self.df_future.loc[idx,'진입수량']:
            매도수량 = self.df_future.loc[idx,'진입수량']
            매수횟수 = 0
        category = 'linear'
        매도수량 = self.common.amount_precision(market, category, ticker, 매도수량)
        print(f"close_linear 청산신호 : {idx}, {매도수량=}, {category=}, {현재가= }, {수익률=}   현재시간: {datetime.datetime.now()}  ")
        res = self.common.order_close(market=market, category=category, ticker=ticker, side=direction,
                                            orderType='market', price=현재가, qty=매도수량)
        id = res['id']
        QTest.qWait(1000)
        i = 0

        while True:
            dict_chegyeol = self.fetch_order(market=market, ticker=ticker, id=id, category=category, qty=매도수량)
            ...
            if dict_chegyeol['체결'] == True:
                ...
                self.df_closed.loc[id, 'category'] = 'linear'
                self.df_closed.loc[id, '주문수량'] = 매도수량
                ...
                if self.df_linear.loc[idx,'매수횟수'] == 0:
                    ...
                    self.df_linear.loc[idx,'주문수량'] = 매도수량
                    self.df_linear.loc[idx,'체결가'] = dict_chegyeol['체결가']
                    self.df_linear.loc[idx,'진입수량'] = 기존진입수량 - dict_chegyeol['진입수량']
                    self.df_linear.loc[idx,'매수금액'] = round(기존매수금액 - (dict_chegyeol['체결금액'] / 레버리지))
                    평단가 = self.df_linear.loc[idx,'매수금액'] / self.df_linear.loc[idx,'진입수량']
                    평단가 = self.common.price_to_precision(market, 'linear', ticker, 평단가)
                    self.df_linear.loc[idx,'평단가'] = 평단가
                    self.df_linear.loc[idx,'수수료'] = 기존수수료 + dict_chegyeol['수수료']
                    self.df_linear.loc[idx,'체결시간'] = dict_chegyeol['체결시간']
                ...
                print(f'{market} | close_linear 에러 1')
                quit()
            QTest.qWait(1000)
            i = i + 1
        ...
        self.fetch_balance()
        self.buy_manual(market=market,ticker=ticker,bet_amount=usdt,df_inverse=self.df_inverse)
...
#             print(f'주문취소 - {market= } | {ticker= } | {category } | {qty= } | {id= }')
            ...
            dict_txt[id]['체결'] = '주문취소'
            dict_txt[id]['체결수량'] = 0
        elif self.common.get_funding_time(주문시간) < datetime.datetime.now(): # 펀딩비시간까지 체결안되면 취소
#             print(f'주문취소 - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M")} | {market= } | {ticker= } | {category= } | {qty= } | {id= }')
            ...
            if category == 'spot':
                ...
                self.buy_manual(market=market,ticker=ticker,bet_amount=배팅금액,df_usdt=self.df_inverse)
    ...
    def get_open_long_signal(self,df,market,ticker):
        today_now = datetime.datetime.now()
        if f"{market}_{ticker}_long" in self.df_linear.index.tolist():
            idx = f"{market}_{ticker}_long"
            진입횟수 = self.df_linear.loc[idx,'매수횟수']
            수익률 = self.df_linear.loc[idx,'수익률']
            체결시간 = pd.to_datetime(self.df_linear.loc[idx,'체결시간'])
        # elif f"{market}_{ticker}_short" in self.df_linear.index.tolist():
        #     idx = f"{market}_{ticker}_short"
        #     진입횟수 = self.df_linear.loc[idx,'매수횟수']
        #     수익률 = self.df_linear.loc[idx,'수익률']
        #     체결시간 = pd.to_datetime(self.df_linear.loc[idx,'체결시간'])
        else:
            진입횟수 = 0
            수익률 = 0
            체결시간 = today_now-datetime.timedelta(days=1) #진입시간이 하루이상 경과되야 추가진입 가능
        signal = False
        신호번호 = 0
        # return signal
        if today_now-체결시간 < datetime.timedelta(days=1):
            print(f'아직 진입 시간이 얼마 안됨 {market=}  {ticker=}| 현재시간: {today_now} | 체결시간: {체결시간}')
        else:
            if 진입횟수 < 2:
                if ((df.loc[df.index[-3],'이평9'] < df.loc[df.index[-3],'이평20']) and
                        (df.loc[df.index[-2],'이평9'] > df.loc[df.index[-2],'이평20'])):
                    signal = True
                    신호번호 = 1
                ...
                if (df.loc[df.index[-3], 'RSI14'] < 70) and (df.loc[df.index[-2], 'RSI14'] > 70):
                    signal = True
                    신호번호 = 3
            ...
            if 진입횟수 < 7:
                if (df.loc[df.index[-3],'RSI30'] > 15) and (df.loc[df.index[-2],'RSI30'] < 15):
                    signal = True
                    신호번호 = 9



        return signal,진입횟수,신호번호

    def get_open_short_signal(self, df, market, ticker):
        if f"{market}_{ticker}_short" in self.df_linear.index.tolist():
            idx = f"{market}_{ticker}_short"
            진입횟수 = self.df_linear.loc[idx, '매수횟수']
            수익률 = self.df_linear.loc[idx, '수익률']
            ...
            진입횟수 = 0
            수익률 = 0
            체결시간 = datetime.datetime.now() - datetime.timedelta(days=1)
        signal = False
        return signal

    def get_close_long_signal(self,df, market,ticker):
        idx = f"{market}_{ticker}_long"
        수익률 = self.df_linear.loc[idx,'수익률']
        수익금 = self.df_linear.loc[idx,'수익금']
        진입수량 = self.df_linear.loc[idx,'진입수량']
        ...
        신호번호 = 0
        signal = False
        if datetime.datetime.now()-체결시간 < datetime.timedelta(hours=4): #매수 후 4시간 이내면 매도 안함
            signal = False
            pass
        elif 수익률 > 100:
            signal = True
            ...
            if df.loc[df.index[-3], 'RSI'] > 70 and df.loc[df.index[-2], 'RSI'] < 70:
                signal = True
                매도수량 = 진입수량
                매수횟수 = 0
                신호번호 = 4

        return signal, 매도수량, 매수횟수, 신호번호
    def get_close_short_signal(self,df,market,ticker):
        signal = False
        idx = f"{market}_{ticker}_long"
        수익률 = self.df_linear.loc[idx,'수익률']
        수익금 = self.df_linear.loc[idx,'수익금']
    ...
    def fetch_order(self, market, ticker, id, category, qty): #주문이 있는데도 조회를 못하는 경우가 있어서 전부다 확인
        ord_open = self.common.fetch_open_orders(market=market, category=category, ticker=ticker, id=id)
        if ord_open == False:
            ord_closed = self.common.fetch_closed_orders(market=market, category=category, ticker=ticker, id=id)  # open 주문과 close 주문 2중으로 확인
            if ord_closed == False:
                ord_canceled = self.common.fetch_cancel_orders(market=market, category=category, ticker=ticker, id=id)  # open 주문과 close 주문 2중으로 확인
                if ord_canceled:
                    return {'체결': '주문취소'}
                else:
                    return {'체결': '보류'}
                ...
                if ord_closed['info'].get('status')=='FILLED' or ord_closed['info'].get('orderStatus')=='Filled': #바이낸스 == 'status', 바이비트 == 'orderStatus'
                    ...
                    dict_info = {'체결': True, '체결가': 진입가, '체결수량':체결수량,'체결금액':체결금액, '수수료':수수료,
                                 '체결시간':체결시간, 'id':id,'side':ord_closed.get('side',None),"ord_closed":ord_closed}
...
class Window(QMainWindow):
    manual_buy_signal = pyqtSignal(str,str,int,pd.DataFrame)
    ...
    def set_UI(self):
        ...
        dict_grid = {
            ...
            QLabel('linear_손익(USD)'):self.QL_linear_benefit,
    ...
    def init_file(self):
        ...
        if not os.path.isfile(db_file): #파일이 없으면
            ...
            self.df_linear = pd.DataFrame(columns=['market','ticker', 'category', '주문가','체결가','평단가', '주문수량',
                                                   '진입수량', '매수금액', '수수료', '체결시간', '매수횟수','레버리지','방향',
                                                   ],)
            ...
            self.df_set = pd.DataFrame(index=['auto_start','auto_finish','start_time','rate_short','funding_time',
            ...
            self.df_set.loc['rate_short','val'] = 0.1
            self.df_set.loc['funding_time','val'] = common_define.get_funding_time(self,datetime.datetime.now())
            ...
            self.df_set.to_sql('set', self.conn, if_exists='replace')
            self.df_top_ticker = pd.DataFrame(index=['bybit','binance'], columns=['market','ticker','합계(USD)','목표(USD)','상태','주문_category','주문수량','주문가','id','주문시간','만료시간'])
            self.df_top_ticker['주문시간'] = datetime.datetime.now().replace(microsecond=0)
            self.df_top_ticker['만료시간'] = datetime.datetime.now().replace(microsecond=0)
            self.df_top_ticker['ticker'] = 'USDT'
            self.df_top_ticker.loc['bybit','market'] = 'bybit'
            self.df_top_ticker.loc['binance','market'] = 'binance'
            self.df_top_ticker.to_sql('top_ticker', self.conn, if_exists='replace')
        else:
            self.conn = sqlite3.connect(db_file)
            ...
            self.df_top_ticker = pd.read_sql(f"SELECT * FROM 'top_ticker'", self.conn).set_index('index')
            funding_time = pd.to_datetime(self.df_set.loc['funding_time', 'val'])
            ...
            # self.df_linear['진입수량'] = self.df_linear['보유수량']
    ...
    def onStartButtonClicked(self):
        ...
        dict_option = {'인버스호가':rate_short,'start_time':start_time}
        self.thread = do_trade(self,self.dict_bybit,self.dict_binance,self.df_open,self.df_closed,self.df_set,self.df_linear,dict_option,self.df_top_ticker)
        self.thread.start()
        ...
        self.thread.q_message.connect(self.text_message)
        self.thread.val_ror_sum.connect(self.QL_linear_benefit.setText)
        self.thread.save_auto_top.connect(self.save_df_top)
    ...
    def request_buy(self,market):
        ...
        # rate_spot = float(self.QL_rate_spot.text())
        self.manual_buy_signal.emit(market,ticker,배팅금액,self.df_qtable_have)

    def save_set(self,df):
        df.to_sql('set',self.conn,if_exists='replace')
    def save_df_top(self,df):
        df.to_sql('top_ticker',self.conn,if_exists='replace')
    def qtable_inverse(self,df):
        if not df.empty:
            ...
            df = df[['market', 'ticker','합계(USD)', 'free(qty)', 'used(qty)', 'total(qty)', '현재가',
                     '배팅가능합계(USD)', '주문최소금액(USD)']]
    ...
    def qtable_future(self,df):
        if not df.empty:
            ...
            df = df[['market','ticker','수익률','수익금','매수금액','현재가', '진입가','진입수량', '방향','레버리지','청산가']]
    ...
    def qtable_linear(self,df):
        ...
        if not df.empty:
            if '수익률' in df.columns.tolist():
                df = df[['market', 'ticker', '수익률','수익금','매수금액','현재가','평단가','진입수량','방향', '레버리지','매수횟수',
                         'category', '주문가', '체결가', '주문수량', '수수료', '체결시간']]
            else:
                df = df[['market', 'ticker', '매수금액','평단가','매수횟수','진입수량','방향',
                         'category', '주문가', '체결가', '주문수량', '수수료', '체결시간', '레버리지']]
        else:
            df = pd.DataFrame()
    ...
    def view_chart(self,market):
        ...
        list_out = []
        self.df_inverse, self.df_future, assets_binance, assets_bybit = self.defi.get_all_assets()
        if not self.df_inverse.empty:
            df_inverse = self.df_inverse[self.df_inverse['market']==market]
            df_inverse.index = df_inverse['ticker']
            df_inverse = df_inverse[['합계(USD)']]
        for i, ticker in enumerate(list_inverse):
            df = self.defi.fetch_funding_rates(market=market, ticker=ticker, since=since)
            df.index = df.index // 1000
            print(df.head(1))  # print를 안하면 데이터에 Nan 이 섞여서 출력됨
            # 중복 인덱스가 있는지 확인
        ...
        if has_duplicates:
            ...
            print(df_ma)
            # print(df_inverse)
            # df_ma = pd.concat([df_ma,df_inverse],axis=1)
            # print(f"{list_out= }")
            ema_values = {}
            ...
            # df_ema = pd.concat([df_ema,df_inverse],axis=1)
            print(df_ema)

            df = pd.concat([df_ma,df_ema],axis=1)
            df['result']=(df['단순평균']+df['지수이동평균'])
    ...
    def make_exchange_bybit_ccxt(self):
        ...
        if api == None or secret == None:
            ...
            try:
                ...
                active = True
                # bybit.set_margin_mode('cross', 'USDT')
            except:
                bybit = None
                session = None
                active = False
...
class common_define():
    ...
    def fetch_tickers(self,market):
        if market == 'bybit':
            ...
            try:
                ...
                print('fetch_tickers - binance 에러')
                return
    ...
    def order_open(self, market, category, ticker, side, orderType, price, qty, leverage=1,df=pd.DataFrame()):
        ...
        if market == 'bybit':
            if category == 'spot':
                ...
                params = {'positionIdx': 0, 'tradeMode': 0}  # [positionIdx - 0 One-Way Mode, 1 Buy-side, 2 Sell-side]  [tradeMode - 0:Isolated,1:cross
                symbol = ticker +'USD'
                leverage = 1
        ...
        if market == 'bybit':
            if category == 'spot':
                ...
                if side == 'short':  # 지정가 close short
                    ...
                    params = {'positionIdx': 1}  # 0 One-Way Mode, 1 Buy-side, 2 Sell-side
            try:
                res = self.dict_bybit['exchange'].create_order(symbol=symbol, type=orderType, side=side, amount=qty,price=price, params=params)
            except:
                print(f"order_close 주문에러  {symbol=}  {category=}  {orderType=}  {side=}   {qty=}   {price=}    {params=}")
                res = self.dict_bybit['exchange'].create_order(symbol=symbol, type=orderType, side=side, amount=qty,price=price, params=params)
            ...
            if category == 'spot':
                ...
                if side == 'long':
                    ...
                    params = {'positionSide': 'LONG'}
                    pass
                elif side == 'short':  # 지정가 close long
                    side = 'buy'
                    params = {'positionSide': 'SHORT'}
                    pass
            try:
    ...
    def order_cancel(self, market, category, ticker, order_id):
        if market == 'bybit':
            if category == 'inverse':
                ...
                symbol = ticker + 'USDT'
            print(f"{symbol=}   {market=}   {category=}   {ticker=}   {order_id=}")
    ...
    def fetch_open_orders(self, market, category, ticker, id):  # 미체결주문 조회
        # try:
        params = {}
        if market == 'bybit':
            if category == 'spot':
        ...
        for order in res:
            if order['id'] == id:
                return order
        return False
    def fetch_closed_orders(self,market, category, ticker, id):  # 체결주문 조회
        params = {}
        if market == 'bybit':
            if category == 'spot':
                symbol = ticker + '/USDT'
        ...
        for order in res:
            if order['id'] == id:
                return order
        return False
    def fetch_cancel_orders(self,market, category, ticker, id):  # 체결주문 조회
        params = {}
        if market == 'bybit':
            if category == 'spot':
                symbol = ticker + '/USDT'
    ...
    def amount_precision(self,market, category, ticker, amount):
        if market == 'bybit':
            if category == 'spot':
                symbol = ticker + '/USDT'
            elif category == 'inverse':
                ...
                symbol = ticker + 'USDT'
            return float(self.dict_bybit['exchange'].amount_to_precision(symbol=symbol,amount=float(amount)))
        elif market == 'binance':
            if category == 'spot':
                symbol = ticker + '/USDT'
            elif category == 'inverse':
                ...
                symbol = ticker + 'USDT'
            return float(self.dict_binance['spot'].amount_to_precision(symbol=symbol, amount=float(amount)))
    ...
    def price_to_precision(self, market, category, ticker, price):
        if market == 'bybit':
            ...
            try:
                ...
                print(f"[price_to_precision] {market= }, {category= }, {ticker= }, {price= }, {symbol= }")
                return float(self.dict_bybit['exchange'].price_to_precision(symbol=symbol, price=price))
        elif market == 'binance':
            if category == 'spot':
                symbol = ticker+'/USDT'
    ...
    def transfer_to(self,market, ticker, amount ,departure , destination):
        if market == 'bybit':
            ...
            try:
                ...
                print(f"자금 이동 중 오류 발생 [transfer_to]: {e} {ticker= } {amount= }, 출발: {departure}, 도착: {destination}")
                #     return False
    ...
    def get_funding_time(self,now):
        ...
        funding_time = today + datetime.timedelta(days=1, hours=funding_hours[0] - today.hour)
        return funding_time
    def fetch_position(self,market,category,ticker):
        if market == 'bybit':
            if category == 'inverse':
                symbol = ticker + 'USD'
                return self.dict_bybit['exchange'].fetch_positions([symbol])[0]
            elif category == 'spot':
                return self.dict_bybit['exchange'].fetch_balance({'type': 'spot'})[ticker]
    ...
    def get_df(self,market,ticker,bong,since_day):
        dict_bong_stamp = {'1분봉': 1 * 60, '3분봉': 3 * 60, '5분봉': 5 * 60, '15분봉': 15 * 60, '30분봉': 30 * 60,
                           '60분봉': 60 * 60, '4시간봉': 240 * 60, '일봉': 1440 * 60, '주봉': 10080 * 60}
        ...
        for market in ['binance','bybit']:
            ...
            if market == 'binance':
                if self.dict_binance['active'] == True:
                    ...
                    df_binance.loc['USDT','free'] = USDT_free  # 각 열에 맞는 값 입력
                    df_binance.loc['USDT','used'] = USDT_used+USDT_USD  # 각 열에 맞는 값 입력
                    df_binance.loc['USDT','total'] = USDT_free+USDT_used+USDT_USD  # 각 열에 맞는 값 입력
                    df_binance.loc['USDT','price'] = 1  # 각 열에 맞는 값 입력
                    df_binance.loc['USDT','주문최소금액(USD)'] = 1  # 각 열에 맞는 값 입력
    ...
    def get_coinm_futures_balance(self):
        ...
        for currency, amounts in balance.items():
            if currency not in ['info', 'free', 'used', 'total', 'timestamp', 'datetime']:
                if amounts['total'] > 0:
                    dict_balance[currency] = amounts
                    if f"{currency}/USD:{currency}" in list(tickers.keys()): #usdm에 종목이 있으면
                        ...
                        dict_balance[currency]['price'] = 0
                        USDT = 0
                        list_del_ticker.append(currency)

        df = pd.DataFrame.from_dict(dict_balance, orient='index')
        for ticker in list_del_ticker:
    ...
    def get_unified_balance(self): #바이비트
    ...
    # def fetch_trade_volume_bybit(self):
    #     tickers = self.dict_bybit['exchange'].fetch_tickers()
    #
    #     # 거래대금 계산 및 정렬
    #     volume_ranking = []
    ...
    #         if '/USDT' in symbol and ':USDT' not in symbol:  # Spot만 (선물 제외)
    #             # 거래대금 = 24시간 거래량 * 현재가
    #             quote_volume = ticker.get('quoteVolume', 0)  # USDT 기준 거래대금
    #
    ...
    #
    #     # 거래대금 순으로 정렬
    #     volume_ranking.sort(key=lambda x: x['volume_usdt'], reverse=True)
    #
    #     # 상위 20개 출력
    #     print("=== 24시간 거래대금 순위 (USDT) ===")
    #     for i, coin in enumerate(volume_ranking[:20], 1):
    #         print(f"{i:2d}. {coin['symbol']:15s} ${coin['volume_usdt']:>15,.0f} (가격: ${coin['price']:,.2f})")
    def get_inverse_TOP(self,market,days,ratio,top):
        # self.fetch_trade_volume_bybit()
        list_inverse = self.fetch_inverse_list(market)
        df_funding = pd.DataFrame()
        since = datetime.datetime.now() - datetime.timedelta(days=days)
        since = self.datetime_to_stamp(since) * 1000  # 밀리초 곱하기
        df = self.fetch_funding_rates(market=market, ticker='BTC', since=False)
        df.index = df.index // 1000
        btc_date_end = df.index[-1]  # 최근 일
        # list_out = []
        self.df_inverse, self.df_future, assets_binance, assets_bybit = self.get_all_assets()
        if not self.df_inverse.empty:
            df_inverse = self.df_inverse[self.df_inverse['market'] == market]
            df_inverse.index = df_inverse['ticker']
            # df_inverse = df_inverse[['합계(USD)']]
        for i, ticker in enumerate(list_inverse):
            df = self.fetch_funding_rates(market=market, ticker=ticker, since=since)
            df.index = df.index // 1000
            print(df.head(1))  # print를 안하면 데이터에 Nan 이 섞여서 출력됨
            # 중복 인덱스가 있는지 확인
            print(f"{ticker}: df_funding 중복 인덱스:", df_funding.index.duplicated().any())
        ...
        if has_duplicates:
            ...
            df_funding = df_funding*10000 # 소숫점이 너무 길어서 모든 값에 1000 곱하기

            #####위 아래로 너무 튀는값은 무시하기 위해 표준편차 기준으로 값을 0으로 바꿈
            # 방법 1: z-score 사용 (표준편차의 배수)
            threshold = 2  # 평균으로부터 표준편차의 2배를 벗어나는 값
            # 각 열의 평균과 표준편차 계산
            mean = df_funding.mean()
            std = df_funding.std()
            print(f"{std=}")
            # z-score 계산
            z_scores = np.abs((df_funding - mean) / std)
            # threshold를 넘는 값을 0으로 변경
            df_funding[z_scores > threshold] = 0
            ...
            list_ma10 = df_ma.head(10).index.tolist()
            list_cross = list(set(list_ema10) & set(list_ma10))  # ema 와 ma의 상위 10개 ticker의 교집합
            print(f"{market}: ema 와 ma의 상위 10개 ticker의 교집합 = {list_cross}")
            df = df.sort_values('result', ascending=False).head(top)
            df = df.loc[df['result']>ratio]  # result가 top 보다 높은거만
            df_ma = df_ma.transpose()
            df_ema = df_ema.transpose()
            df_funding = pd.concat([df_ma,df_funding],axis=0)
            df_funding = pd.concat([df_ema,df_funding],axis=0)
            df_funding.index = df_funding.index.astype(str)
            con = sqlite3.connect('DB/Funding_Strategy.db')
            df_funding.to_sql("funding_rate_",con,if_exists='replace')
            con.close()
...
if __name__ == "__main__":
    ...
    #                     # df = df[['symbol', '현재가', '레버리지', '방향', '수익률', '손익', '진입수량', '매수금액', '진입가', '청산가', 'marginMode']]
    #                     df = df[common_col]
    #
    ...
    #                     # df.rename(columns={'unrealizedPnl': '손익', 'leverage': '레버리지', 'contracts': '진입수량',
    ...
    #         self.df_future.rename(columns={'unrealizedPnl': '손익', 'contracts': '진입수량',
