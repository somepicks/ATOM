...
class Trade_np(QThread):
    ...
    def __init__(self,parent,market,simul,df_stg,chart_duration,tele,list_tickers):
    ...
    def loop_init(self):
        ...
        if self.market == '코인':
            ...
            if self.market == '국내주식':
                ...
                # self.list_tickers.append('콜옵션')
                # self.list_tickers.append('풋옵션')
        ...
        for stg in self.df_trade.index:
            ...
            if type(obj) == str or (type(obj) != str and 상태 != '대기'): #종목이 지정되어있을 경우
                ...
                if 상태 == '대기':
                    self.df_trade.loc[(self.df_trade['ticker'] == ticker) & (self.df_trade['봉'] == bong) & (self.df_trade['상태'] == '대기') & (self.df_trade['상세봉'] == bong_detail), '진입시간'] = \
                                        common_def.datetime_to_str(df.index[idx_bong])  # 변수로 사용하기 때문에 일단은 만들어둠
                    self.df_trade.loc[(self.df_trade['ticker'] == ticker) & (self.df_trade['봉'] == bong) & (self.df_trade['상태'] == '대기') & (self.df_trade['상세봉'] == bong_detail), '현재봉시간'] = \
                                        common_def.datetime_to_str(df.index[idx_bong])  # 조건: 같은 종목, 같은 봉, 같은 봉제한인 테이블에 동시에 저장
                ...
                if self.list_obj:
                    for i, ticker in enumerate(self.list_obj):  # 조건 검색에 있는 종목만
                        ...
                        df_same = self.df_trade[(self.df_trade['ticker'] == ticker) & (self.df_trade['봉'] == bong) & (
                                    self.df_trade['상세봉'] == bong_detail)]
                        if df_same.empty:
            ...
            if [x for x in df.columns.tolist() if '_y' in x or '_x' in x]:
                print('에러0')
                quit()
        one_minute = datetime.datetime.now()
        ...
        if self.market == '코인':
            ...
            while self._status:
                frgn, prsn, orgn = self.loop_main(장종료시간, frgn, prsn, orgn, one_minute)
                self.qt_open.emit(self.df_trade)
                # self.val_light.emit(self.bool_light)
                # self.bool_light = not self.bool_light
    ...
    def loop_main(self,장종료시간, frgn, prsn, orgn, one_minute):
        ...
        if 장종료시간 < 현재시간:
            self.market_finish()
        # print(self.df_trade[['현재봉시간']])
        elif 현재시간 - one_minute > datetime.timedelta(minutes=1):  # 모든종목 대상으로 캔들종료시간 초과 시 한번만 실행하도록
            # global frgn, prsn, orgn
            if self.market == '국내주식' or self.market == '국내선옵':
                frgn, prsn, orgn = self.ex_kis.investor_trend_time('선물')
                외인 = frgn
                개인 = prsn
        ...
        for stg in self.df_trade.index:
            ...
            if type(obj) == str or (type(obj) != str and 상태 != '대기'):  # 종목이 지정되어있을 경우
                ...
                self.list_df_duplicated.append(ticker_full_name)
                # else:
                #     ohlcv = globals()[f'{ticker}_{bong}_{bong_detail}']
                #     print(f'{ticker}_{bong}_{bong_detail}')
                #     quit()
                #     if (self.market == '국내주식' or self.market == '국내선옵') and bong != '일봉':
                ...
                for i, ticker in enumerate(self.list_obj):  # 조건 검색에 있는 종목만
                    ticker_full_name = f'{ticker}_{bong}_{bong_detail}'
                    if not ticker_full_name in self.list_df_duplicated:  # 기존에 만들어진 df 없으면
                        df = self.make_df(ticker_full_name, ticker, bong, bong_detail, bong_since)
                        self.list_df_duplicated.append(ticker_full_name)
                        if i == 0:  # 여러종목일 경우 첫번째로 불러오는 데이터의 봉 시간만 저장
                            self.df_trade.loc[stg, '현재봉시간'] = common_def.datetime_to_str(df.index[-1])

                    elif ticker_full_name in self.list_df_duplicated:
                        list_df = globals()[f'{ticker}_{bong}_{bong_detail}']
                        stamp_date_old = list_df[-1][0] / 1000 + self.dict_bong_stamp[bong_detail]  # 다음봉 시간 계산
                        if stamp_date_old < time.time():  # detail봉 시간 기준 데이터 생성
                            df = self.make_df(ticker_full_name, ticker, bong, bong_detail, bong_since)
                            if i == 0:  # 여러종목일 경우 첫번째로 불러오는 데이터의 봉 시간만 저장
                                self.df_trade.loc[stg, '현재봉시간'] = common_def.datetime_to_str(df.index[-1])
                        else:
                    ...
                    if not df.empty:
                        데이터길이 = df.loc[df.index[-1], '데이터길이']  # df는 상태봉이기 때문에  찾아서 다시 들어가야됨
                        # idx_bong = df['데이터길이'].tolist().index(데이터길이)
                        #진입대상이 전체인 전략에서 이미 매수를 한 종목은 다시 매수하지 않음
                        self.loop_trade(ticker, stg, df, bong, bong_detail, 상태, 현재시간, 캔들종료시간, 장종료시간,데이터길이,배팅금액, 진입시간, frgn, prsn, orgn)

            self.active_light()
        return frgn, prsn, orgn
        # print(self.df_trade[['현재봉시간','캔들종료시간']])
        # quit()

    def loop_trade(self, ticker, stg, df_detail, bong, bong_detail, 상태, current_time, candle_endtime, jang_endtime,data_length,배팅금액,enter_time, frgn,prsn,orgn):
        global np_tik_ar, list_columns, np_tik_idx
        global np_tik_length
        global 매수가, 매도가, 시장가, 레버리지, 매수, 매도
        global 수익률, 최고수익률, 최저수익률
        global 상세시가, 상세고가, 상세저가, 상세종가
        global market, 전략명, 종목코드
        global dict_bong_stamp, bong_stamp, bong_detail_stamp
        global 롱, long, 숏, short
        global 분봉1, 분봉3, 분봉5, 분봉15, 분봉30, 분봉60, 시간봉4, 일봉, 주봉, 월봉
        global 현재가, 시가, 고가, 저가, 종가, 거래량, NAV, 거래량이평3, 이격도20이평, 등락율, 시가총액, 이평
        global 기관, 외인, 개인
        global 캔들종료시간, 장종료시간, 데이터길이, 진입시간, 현재시간
        global 분할상태, 매입율
        현재시간 = current_time
        ...
        # 매수 = True
        # 매도 = True
        현재가 = df_detail.loc[df_detail.index[-1],'종가']
        # 현재시간 = datetime.datetime.now()
        ...
        if 상태 == '대기' or 상태 == '분할매수대기': # 미 보유 시 진입 주문
            ...
            self.df_trade.loc[stg, '현재가'] = 현재가
            print(f"elif 상태 == '매수주문' or 상태 == '부분매수' {stg= }     {ticker=  }     {상태= }   {분할상태=}" )
            if 상태 == '분할매수주문' or 상태 == '분할부분매수' or 상태 == '분할매수취소': #분할매수일 경우
                self.chegyeol_buy_on_scale(stg, ticker, 방향, 현재시간, 상태)
            else:
                ...
                상태, 보유수량, 매입금액, 총진입수수료, 진입가 = self.chegyeol_buy(stg, ticker, 방향, 진입가, 현재시간, 상태, 잔고,누적진입수수료,주문수량,보유수량,매입금액,id)
            ...
            print(f"elif 상태 == '매수' {전략명= }    {종목코드=}    {상태= }     {self.df_trade.loc[stg, '매도전환']}")
            if self.df_trade.loc[stg, '매도전환'] == 'True': #매수가되면 분할매도 때문에 최초 분할 수량을 배정함
                ...
                상태 = '매수'
                self.df_trade.loc[stg, '상태'] = 상태
                print(f"if self.df_trade.loc[stg, '매도전환'] == True:  {stg= }    {ticker= }  {상태= } {분할상태= }  {self.df_trade.loc[stg, '매도전환']= }")
            # self.df_trade.loc[stg, '분할상태'] = json.dumps(['매수'], ensure_ascii=False) # chegyeol_sell에서 분할매도인지 아닌지 확인하기 위해 리스트원소를 1개로 초기화 해줌
            print(f"elif 상태 == '매수' or {stg= }    {ticker= }   {분할상태= }    {self.df_trade.loc[stg, '매도전환']= }   {type(self.df_trade.loc[stg, '매도전환'])= }")
            self.df_trade.loc[stg, '현재가'] = 현재가
            ...
            if 매도 == True or type(매도)==list:  # 청산 주문
                ...
                print(f"if 매도 == True or type(매도)==list: {stg= }    {ticker= }   {분할상태= }   {매도=}    {매도가= }   {type(매도가)= }")
            ...
            self.df_trade.loc[stg, '현재가'] = 현재가
            print(f"loop_trade  -   {stg= }     {ticker= }    {상태= }    {self.df_trade.loc[stg, '분할상태']}   {self.df_trade.loc[stg, '분할보유수량']} ")
            # 수익금, 누적수익금 = self.cal_ror(stg, 현재가, 방향)
            if 상태 == '분할매도주문' or 상태 == '분할부분매도' or 상태 == '분할매도취소': #분할매도일 경우
                상태, 누적수익금 = self.chegyeol_sell_on_scale(stg, ticker, 방향, 현재시간, 상태)
            else:
                보유수량 = self.df_trade.loc[stg, '보유수량']
                매입금액 = self.df_trade.loc[stg, '매입금액']
                ...
                상태, 누적수익금, 보유수량, 평가금액, 청산금액 = \
                    self.chegyeol_sell(stg, ticker, 방향, 상태, 보유수량, 잔고, 현재가, 현재시간,매입금액,
            ...
            if 매도 == True :
                if 매도가 == 시장가 and 상태 != '시장가매도' and 상태 != '매도': # 지정가로 매도주문이 나간상태인데 손절라인에 걸려서 시장가 매도 주문이 나가야할 경우
                    print(f'{stg= }, {ticker= } | 잔량 시장가 매도 - {datetime.datetime.now()}')
                    주문수량 = self.df_trade.loc[stg, '주문수량']
                    보유수량 = self.df_trade.loc[stg, '보유수량']
                    if 상태 == '매도주문' or 상태 == '부분매도': #분할매도 아닐 경우
                        id = self.df_trade.loc[stg, 'id']
                        상태, 잔여수량 = self.order_cancel(stg, ticker, 상태, 주문수량, 보유수량, 방향,id, 0,'')
                    else: #분할매도일 경우
                        print('분할매도 시장가 손절')
                        분할상태 = json.loads(self.df_trade.loc[stg, '분할상태'])
                        ...
                        for num, 세부상태 in enumerate(분할상태):
                            if 세부상태 == '매도주문' or 세부상태 == '시장가매도' or 세부상태 == '부분매도' :
                                상태, 세부잔여수량 = self.order_cancel(stg, ticker, 세부상태, 분할주문수량[num], 분할보유수량[num], 방향,분할id[num], 0,'')
                                잔여수량 += 세부잔여수량
    ...
    def order_buy(self, stg, 매수, 매수가, 방향, 레버리지, 배팅금액, 상태):
        ...
        if self.market == '코인':
            ...
            if self.market == '국내주식':
                ...
                trade_market = '선물' if ticker[:1] == '1' else '콜옵션' if ticker[:1] == '2' else '풋옵션' if ticker[:1] == '3' else '스프레드'
    ...
    def check_compare_ticker(self,stg):
        ...
        for ticker in self.list_tickers:
            ...
            if ticker in stg_buy or ticker in stg_sell:
                while ticker in stg:
                    stg = stg[stg.index(ticker):]
                    if '봉' in stg:
                        bong_len = stg.index('봉')
                    else:
                        bong_len = 0
                    if bong_len > 10:
                        stg = stg[1:]
    ...
    def add_compare_df(self, ticker_main, df, bong_main, list_compare,bong_detail,bong_since):
        ...
        for com_ticker_bong in list_compare:
            ...
            # if not f'{ticker_full_name}' in self.list_df_duplicated:  # 만들어진 데이터가 없을 경우
            #     self.list_df_duplicated.append(ticker_full_name)
            ...
            if bong != '일봉' and bong != '주봉': #분봉일 경우
                ...
                # self.test_sql(df_compare,'df_compare')
                df = df.merge(df_compare, on='date',how='left', suffixes=('', '_daily')) # how='left' df_compare에 값이 없을 경우 nan
                # self.test_sql(df,'df_result')
                # df.to_sql('df_result', sqlite3.connect('DB/bt.db'), if_exists='replace')
    ...
    def sorting_tickers(self,obj):
        ...
        if self.market == '코인':
            ...
            self.list_obj_all = list(set(list_increase_vol) & set(list_top_vol))  # 거래대금 급등, 거래대금 상위 교집합
            # self.list_obj_all = [x[:-10] for x in list_tickers if x[-4:] == 'USDT' and x[:6] != 'GASDAO']  #GASDAO 종목 삭제
            # list_tickers = list(set(list_tickers) - set(self.list_stg_tickers))  # 전체 대상에서 전략에 종목이 지정된 종목은 제외
            print(f"초기 거래대상 종목: {self.list_obj_all}")
        elif self.market == '국내주식':
            # print(self.df_stock_info.sort_values(by=['PER,PBR'],ascending=[True,True]))
            # df = self.df_stock_info[self.df_stock_info['PER']!=0] #per 0 제외
            # df = df[df['PBR']!=0]  #pbr 0 제외
            # df = df.sort_values('PER')
            # df = df[:100] # 상위 100위
            # df = df.sort_values('PBR')
            # df = df[:10] # 상위 50위
            # self.list_obj_all = df.index.tolist()
            self.list_obj = []
            for content in obj:
            ...
            for content in obj:
                print(f"{content= }")
                if content[1:3] == '옵션' and (content[-2:] == '월간' or content[-2:] == '월물'): #옵션_월간
                    print(1)
                    print(self.list_tickers)
                    if '콜' in content:
                        print(f"{content= }")

                    # df_c, df_p = self.ex_kis.display_opt(datetime.datetime.today())
                    # print(df_c)
                    # print(df_p)
                elif content[1:3] == '옵션' and (content[-2:] == '주간' or content[-3:] == '위클리'): #옵션_주간
                    # print(2)
                    # 월: 0, 화: 1 ~ 일: 6
                    week = datetime.datetime.weekday(datetime.datetime.today())

                    if week == 4 or week == 5 or week == 6 or week == 0: #위클리 월요일일 경우
                        df_c, df_p = self.ex_kis.display_opt_weekly_mon()
                    ...
                    # df_c.drop('종목코드',axis=1,inplace=True)
                    # df_p.drop('종목코드',axis=1,inplace=True)
                    # df_c = df_c.apply(to_numeric)
                    # df_p = df_p.apply(to_numeric)
                    # self.test_sql(df_c,'call')
                    # self.test_sql(df_p,'put')
                # df = df_c if content[:1] == '콜' else df_p
                # if content[content.index('_')+1:] == '거래량상위':
                #     df = df[['거래량']].apply(to_numeric)
                #     df.sort_values(by='거래량',ascending=False,inplace=True)
                #     df = df.iloc[:10, :] #상위 10개종목
...
if __name__ == "__main__":
    ...
    if market == '국내주식':
        ...
        if not df_c_w.empty and not df_p_w.empty:
            ...
            df_c_w = df_c_w[df_c_w['행사가'] < 현재가 + 25]
            df_c_w['종목명'] = '콜_'+yoil+'_위클리'

            df_p_w = df_p_w[df_p_w['행사가'] > 현재가 - 25]
            df_p_w = df_p_w[df_p_w['행사가'] < 현재가 + 25]
            df_p_w['종목명'] = '풋_'+yoil+'_위클리'
    ...
    tele = True
    duration = 30
    trade = Trade_np(None,market,simul,df_stg,duration,tele,list_tickers)
